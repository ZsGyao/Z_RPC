/**
  ******************************************************************************
  * @file           : zrpc_pb_rpc_dispatcher.cpp
  * @author         : zgys
  * @brief          : None
  * @attention      : None
  * @date           : 23-3-21
...
#include "src/network/zRpcPb/zrpc_pb_rpc_closure.h"
...
namespace zrpc {
    ...
    void ZRpcPbRpcDispatcher::dispatch(AbstractData* data, TcpConnection* conn) {
        ...
        InfoLog << "begin to dispatch client zRpcPb request, msgno=" << tmp->msg_req;

        std::string service_name;
        std::string method_name;

        ZRpcPbStruct reply_pk;
        ...
        if (it == m_service_map.end() || !((*it).second)) {
            ...
            InfoLog << "end dispatch client zRpcPb request, msgno=" << tmp->msg_req;
            return;
        ...
        ZRpcPbRpcController rpc_controller;
        ...
        ZRpcPbRpcClosure closure(reply_package_func);
    ...
    bool ZRpcPbRpcDispatcher::parseServiceFullName(const std::string& full_name, std::string& service_name, std::string& method_name) {
        if (full_name.empty()) {
    ...
    void ZRpcPbRpcDispatcher::registerService(service_ptr service) {
        std::string service_name = service->GetDescriptor()->full_name();
        m_service_map[service_name] = service;
        InfoLog << "success register service[" << service_name << "]!";
    }
