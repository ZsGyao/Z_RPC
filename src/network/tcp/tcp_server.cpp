...
namespace zrpc {
    extern std::shared_ptr<Config> zRpcConifg;
    ...
    void TcpAcceptor::init() {
        ...
        // assert(m_fd != -1);
        DebugLog << "create listenfd success, listenfd=" << m_fd;
        ...
        DebugLog << "set REUSEADDR success";
        rt = listen(m_fd, 10);
        if (rt != 0) {
    ...
    int TcpAcceptor::toAccept() {
        ...
        InfoLog << "New client accepted success! fd:[" << rt <<  ", addr:[" << m_peer_addr->toString() << "]";
        return rt;
    }
    ...
    TcpServer::TcpServer(NetAddress::ptr addr, ProtocalType type) : m_addr(addr) {
        ...
        m_io_pool = std::make_shared<IOThreadPool>(zRpcConfig->m_io_thread_num);
        if (type == Http_Protocal) {
            ...
            m_dispatcher = std::make_shared<ZRpcPbRpcDispatcher>();
            m_codec = std::make_shared<ZRpcPbCodeC>();
            m_protocal_type = zRpcPb_Protocal;
        }

        m_main_reactor = zrpc::Reactor::GetReactor();
        m_main_reactor->setReactorType(MainReactor);

        m_time_wheel = std::make_shared<TcpTimeWheel>(m_main_reactor, zRpcConfig->m_time_wheel_bucket_num, zRpcConfig->m_time_wheel_interval);

        m_clear_client_timer_event = std::make_shared<TimerEvent>(10000, true, std::bind(&TcpServer::ClearClientTimerFunc, this));
    ...
    void TcpServer::start() {
        ...
        m_acceptor->init();
        m_accept_cor = zRpcCoroutinePool->getCoroutineInstance();
        m_accept_cor->resetCallBack(std::bind(&TcpServer::MainAcceptCorFunc, this));

        InfoLog << "resume accept coroutine";
        m_accept_cor.get()->resume();
    ...
    TcpServer::~TcpServer() {
        zRpcCoroutinePool->returnCoroutine(m_accept_cor);
        DebugLog << "~TcpServer";
    }
    ...
    void TcpServer::MainAcceptCorFunc() {
        while (!m_is_stop_accept) {
            ...
            conn->initServer();
            DebugLog << "tcp connection address is " << conn.get() << ", and fd is" << fd;
    ...
    bool TcpServer::registerService(std::shared_ptr<google::protobuf::Service> service) {
        if (m_protocal_type == zRpcPb_Protocal) {
            if (service) {
                dynamic_cast<ZRpcPbRpcDispatcher*>(m_dispatcher.get())->registerService(service);
            } else {
    ...
    void TcpServer::ClearClientTimerFunc() {
        ...
        for (auto &i : m_clients) {
            ...
            if (i.second && i.second.use_count() > 0 && i.second->getState() == Closed) {
                ...
                DebugLog << "Tcp Connection [fd:" << i.first << "] will delete, state=" << i.second->getState();
                (i.second).reset();
                // s_conn.reset();
            }
